...
export default function LoginPage() {
  ...
  // แก้ไขตรงนี้: เปลี่ยนเป็น async เพื่อรอข้อมูลจากระบบหลังบ้าน (Django)
  const handleSubmit = async (e: React.FormEvent) => {
      e.preventDefault();
      setError('');

      // identifier ในที่นี้คือค่าที่เรากรอกในช่อง Username
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-blue-100 flex items-center justify-center p-4">
      <div className="w-full max-w-6xl grid md:grid-cols-2 gap-8 items-center">
        ...
        <div className="bg-white rounded-2xl shadow-2xl p-8 md:p-10">
          ...
          {/* ป้าย Demo Accounts ผมปล่อยไว้เผื่อคุณอยากลบทิ้งทีหลังได้ครับ */}
          <div className="mt-8 p-4 bg-blue-50 rounded-lg">
            <p className="text-sm font-medium text-blue-900 mb-2">Notice:</p>
            <div className="space-y-1 text-xs text-blue-700">
              <p>ตอนนี้ระบบเชื่อมต่อกับฐานข้อมูลจริงแล้ว กรุณาใช้อีเมลและรหัสผ่านจากระบบ (n8n)</p>
            </div>
          </div>
        </div>
      </div>
